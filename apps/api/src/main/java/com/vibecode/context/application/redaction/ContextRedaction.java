 ...
 * production class anywhere in the application — not merely in {@code ..context..} — may depend on
 * {@link RedactedContextItem} at all, so no other class can reach {@link
 * RedactedContextItem#producedByRedaction(ContextItem)} by a call or by a method reference. Stated
 * as a dependency and not as a call because the call-shaped version of this rule was walked through
 * with a method reference, and a rule that enumerates access kinds is open to whichever kind is
 * thought of next.
 *
